...
class RemoteRepository {
  ...
  Future<MovieResponseItem> getTrendingMovies() async {
    final MovieResponseModel result =
        await datasource.getTrendingMovies(apiKey);
  ...
  Future<MovieResponseItem> getTopRatedMovies() async {
    final MovieResponseModel result =
        await datasource.getTopRatedMovies(apiKey);
    return MovieResponseItem.fromModel(result);
  }

  // Отримання всіх фільмів з пагінацією
  Future<MovieResponseItem> getAllMovies({required int page}) async {
    final MovieResponseModel result =
        await datasource.getAllMovies(apiKey, page);
    return MovieResponseItem.fromModel(result);
  }

  // Отримання всіх телешоу з пагінацією
  Future<TVShowResponseItem> getAllTVShows({required int page}) async {
    final TVShowResponseModel result =
        await datasource.getAllTVShows(apiKey, page);
    return TVShowResponseItem.fromModel(result);
  }

  // Пошук за запитом з пагінацією
