...
final GetIt serviceLocator =
    GetIt.instance; // Створюємо інстанс контейнера залежностей.
...
Future<void> init() async {
  ...
  final dio = Dio() // Створюємо об'єкт Dio для здійснення HTTP-запитів.
    ..options.headers['Authorization'] =
        'Bearer ${GlobalConstants.authApiKey}' // Додаємо заголовок авторизації.
    ..options.responseType =
        ResponseType.json // Встановлюємо тип відповіді в JSON.
    ..interceptors.add(
      LogInterceptor(
  ...
  final remoteDatasource = RemoteDatasource(
      dio,); // Створюємо віддалене джерело даних з використанням Dio.

  final localDatasource = LocalDatasource(
    // Створюємо локальне джерело даних з використанням Hive.
    serviceLocator<Box>(
        instanceName: GlobalConstants
            .moviesStorage), // Отримуємо доступ до сховища Movies.
    serviceLocator<Box>(
        instanceName: GlobalConstants
            .tvShowsStorage), // Отримуємо доступ до сховища TVShows.
    serviceLocator<Box>(
        instanceName: GlobalConstants
            .ratingStorage), // Отримуємо доступ до сховища Ratings.
  );
  ...
  serviceLocator.registerLazySingleton<MainCubit>(
    // Реєструємо LazySingleton кубіту головного екрану.
    () => MainCubit(
