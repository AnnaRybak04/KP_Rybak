...
class FavoriteCubit extends Cubit<FavoriteState> {
  ...
  void init() async {
    ...
    try {
      final Future<List<MovieItem>> moviesResult =
          repository.getFavouriteMovies();
      final Future<List<TVShowItem>> tvShowsResult =
          repository.getFavouriteTVShows();
      await Future.wait([moviesResult, tvShowsResult]).then((value) {
        final List<MovieItem> movies =
            value[0] as List<MovieItem>; // Отримання списку улюблених фільмів
        final List<TVShowItem> tvShows =
            value[1] as List<TVShowItem>; // Отримання списку улюблених телешоу
        emit(
          state.copyWith(
            status: MainStatus.initial, // Стан змінюється на "ініціалізований"
      ...
      emit(state.copyWith(
          ...
          errorMessage: e
              .toString())); // Стан змінюється на "помилка" з повідомленням про помилку
    }
  }
}
