...
class MainCubit extends Cubit<MainState> {
  MainCubit(this.localRepository, this.remoteRepository)
  ...
  void init() async {
    emit(state.copyWith(
        status: MainStatus.loading)); // Емітує стан зі статусом завантаження
    try {
      // Отримання даних про трендові, популярні та улюблені фільми
      final Future<MovieResponseItem> trendingResult =
          remoteRepository.getTrendingMovies();
      final Future<MovieResponseItem> popularResult =
          remoteRepository.getTopRatedMovies();
      final Future<List<MovieItem>> favouriteMoviesResult =
          localRepository.getFavouriteMovies();
      await Future.wait([trendingResult, popularResult, favouriteMoviesResult])
          .then((result) {
  ...
  void onFavouriteMovie(MovieItem movie) async {
    ...
    final bool isFavourite =
        favouriteMovies.firstWhereOrNull((element) => element.id == movie.id) !=
            null;
    if (isFavourite) {
      await localRepository
          .removeFromFavouriteMovie(movie); // Видаляє фільм з улюблених
    } else {
      await localRepository
          .saveAsFavouriteMovie(movie); // Додає фільм до улюблених
    }

    final List<MovieItem> newFavouriteMovies = await localRepository
        .getFavouriteMovies(); // Оновлює список улюблених фільмів
    emit(state.copyWith(
        favouriteMovies: newFavouriteMovies)); // Емітує оновлений стан
  }
}
