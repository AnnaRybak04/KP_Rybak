...
class MoviesCubit extends Cubit<MoviesState> {
  ...
  void search({required int page}) async {
    emit(state.copyWith(status: MainStatus.loading)); // Встановлюємо статус завантаження.
    try {
      final MovieResponseItem result = await repository.getAllMovies(page: page); // Отримуємо результат пошуку.
      state.moviesPagingController.appendPage(
        // Додаємо сторінку фільмів до списку.
        result.items,
        result.nextPage,
      );
      emit(state.copyWith(status: MainStatus.initial)); // Встановлюємо початковий статус.
    } catch (e) {
      emit(state.copyWith(status: MainStatus.error, errorMessage: e.toString())); // Встановлюємо статус помилки та повідомлення про помилку.
    }
  }
}
